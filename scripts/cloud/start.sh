...
# idempotent and returns once the database is ready; the Django dev server runs
# separately as a long-lived terminal (see .cursor/environment.json).
set -euo pipefail
...
if ! sudo mariadb-admin --socket="${SOCK}" ping >/dev/null 2>&1; then
  ...
  sudo bash -c "nohup mariadbd --user=mysql --port=${DB_PORT} --socket=${SOCK} >/tmp/mariadb.log 2>&1 &"
...
WINE_COUNT="$(mariadb -u root "-p${DB_PASS}" -h 127.0.0.1 -P "${DB_PORT}" -N -B \
